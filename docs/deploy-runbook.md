...
- **vLLM local (CPU mode)**: `make vllm-deploy` applies
  `k8s/vllm/overlays/kind-cpu`. On the current dev CPU the prebuilt vLLM
  CPU image is not runnable (no AVX-512 — see benchmarks.md), so the
  committed way to exercise `USE_LOCAL_MODEL` locally is Ollama via
...
8. **[Phase 2 — NOT YET EXECUTED]** vLLM on the A10: prepopulate the
   `vllm-model` PVC with the merged checkpoint (e.g. a one-shot Job
   pulling from the eval-artifacts bucket), then
   `kubectl apply -k k8s/vllm/overlays/oke-gpu`. Only after vLLM serves
   the model on the A10 may any doc claim it does; update CLAUDE.md at
   that point.

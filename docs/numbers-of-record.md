...
|---|---|---|
| Extended baseline, 40 tickers (2026-09-05, `grounding-eval-extended-9j2dj`) | Judge v2, post-retrieval-fix index: 387 claims (357 S / 12 U / 18 I), **3.10% unsupported (95% CI 1.8–5.3%)** — point estimate below the 5% gate; the interval includes the gate. INFERENCE share 4.7% vs ~20% under v1. 40/40 tickers (the five ADRs ran without SEC context); per-ticker claims ranged 1–23; UPST the outlier at 3/14 unsupported. 27 min, est. $2.35. Superseded as baseline by `j4cnp` (rebuilt image, same rate: 3.06%, Fisher p = 1.0 — run-to-run stability); the number-of-record decision still waits for the held-out judge validation | grounding-eval DAG; artifacts `eval/runs/9j2dj-*`, per-claim rows `eval/runs/9j2dj-claims.jsonl` |
| **40-ticker A/B, hosted vs in-cluster vLLM fine-tune (2026-09-05/06)** | Judge v2, same image + index both arms: baseline `j4cnp` 12/392 = **3.06%** (CI 1.8–5.3%, gate PASSED, est. $2.36) vs local-model `lsnnc` 30/368 = **8.15%** (CI 5.8–11.4%, gate FAILED, est. $2.44), Fisher **p = 0.0023** — the first A/B that separates the arms on its own. Failure concentrates in the fine-tune-owned sections: attributed FH+RF claims 22/111 = 19.82% vs baseline 1/202 = 0.50% (p = 4.6e-10, `eval/section_attribution.py`; heuristic attribution, ~78% coverage); other sections statistically indistinguishable. Five ADRs on stock+news only, both arms. WMT the local outlier at 9/16. The `USE_LOCAL_MODEL` ship-off decision rests on this A/B | grounding-eval DAG; per-claim rows `eval/runs/{j4cnp,lsnnc}-claims.jsonl`; stats `eval/stats.py`, breakdown `eval/section_attribution.py` |

...
|---|---|---|
| Grounding | **12/392 = 3.06% unsupported (Wilson 95% CI 1.8–5.3%)** — hosted baseline `j4cnp` (2026-09-05/06), judge v2, fixed retrieval (post-2026-09-04), 40 tickers. The point estimate is under the 5% gate; the interval reaches it. Judge-v2 rate — approximate per the held-out calibration (75% recall / 60% precision on UNSUPPORTED, blind labels, n=50), a point estimate, not a bound. Supersedes "49% pre-fix → 0/84" (judge v1, pre-retrieval-fix), now a dated record below | grounding-eval DAG `grounding-eval-extended-j4cnp`; per-claim rows `eval/runs/j4cnp-claims.jsonl`; interval: `eval/stats.py` |
...
calibration — never a bare rate, and never a bare "0%".
...
| Test suite | 2027 lines (`wc -l tests/*.py`), 135 tests collected: 134 passed + 1 skipped (the credit-gated judge test) — as of 2026-09-23 | `python -m pytest tests/ --collect-only`; `python -m pytest tests/` |
